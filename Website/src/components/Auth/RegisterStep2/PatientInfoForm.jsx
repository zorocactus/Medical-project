import { useState } from "react";
import { Check, Phone, ChevronDown } from "lucide-react";

// ────────────────────────────────────────────────────────────────────────────────
// Constants
// ────────────────────────────────────────────────────────────────────────────────
const SEXE_OPTIONS = ["Masculin", "Féminin"];

const BLOOD_GROUPS = ["A+", "A−", "B+", "B−", "AB+", "AB−", "O+", "O−"];

const WILAYAS = [
  "Adrar","Chlef","Laghouat","Oum El Bouaghi","Batna","Béjaïa","Biskra",
  "Béchar","Blida","Bouira","Tamanrasset","Tébessa","Tlemcen","Tiaret",
  "Tizi Ouzou","Alger","Djelfa","Jijel","Sétif","Saïda","Skikda",
  "Sidi Bel Abbès","Annaba","Guelma","Constantine","Médéa","Mostaganem",
  "M'Sila","Mascara","Ouargla","Oran","El Bayadh","Illizi",
  "Bordj Bou Arréridj","Boumerdès","El Tarf","Tindouf","Tissemsilt",
  "El Oued","Khenchela","Souk Ahras","Tipaza","Mila","Aïn Defla",
  "Naâma","Aïn Témouchent","Ghardaïa","Relizane",
];

const ALLOWED_KEYS = ["Backspace","Delete","Tab","ArrowLeft","ArrowRight","Home","End"];

// ────────────────────────────────────────────────────────────────────────────────
// Reusable Custom Dropdown — design exact demandé (fixed inset overlay)
// ────────────────────────────────────────────────────────────────────────────────
function CustomSelect({ label, value, options, onSelect, placeholder = "Sélectionner...", error }) {
  const [isOpen, setIsOpen] = useState(false);
  return (
    <div className="relative group">
      <span className="absolute -top-3 left-4 px-1.5 bg-white text-[13px] font-medium text-[#365885] z-10">
        {label}
      </span>
      <div className="relative">
        <button
          type="button"
          onClick={() => setIsOpen(!isOpen)}
          className={`w-full flex items-center justify-between px-4 py-3 rounded-xl border ${
            error ? "border-red-400" : "border-[#D1DFEC] hover:border-[#89AEDB] focus:border-[#6492C9]"
          } bg-white text-sm text-gray-700 transition-colors text-left outline-none`}
        >
          <span className={value ? "text-gray-700" : "text-[#A0B5CD]"}>{value || placeholder}</span>
          <ChevronDown size={18} className={`text-[#89AEDB] shrink-0 transition-transform ${isOpen ? "rotate-180" : ""}`} />
        </button>

        {isOpen && (
          <>
            <div className="fixed inset-0 z-40" onClick={() => setIsOpen(false)} />
            <div className="absolute top-full left-0 right-0 mt-2 rounded-2xl border border-[#D1DFEC] shadow-xl z-50 py-2 bg-white max-h-60 overflow-y-auto">
              {options.map((opt) => (
                <button
                  key={opt}
                  type="button"
                  onClick={() => { onSelect(opt); setIsOpen(false); }}
                  className={`w-full flex items-center px-5 py-3 text-sm font-medium transition-all text-left hover:bg-[#6492C9] hover:bg-opacity-10 ${
                    value === opt ? "text-[#6492C9] bg-[#EEF3FB]" : "text-[#0D2644]"
                  }`}
                >
                  {opt}
                </button>
              ))}
            </div>
          </>
        )}
      </div>
      {error && <p className="text-red-500 text-[12px] mt-1 ml-1">{error}</p>}
    </div>
  );
}

// ────────────────────────────────────────────────────────────────────────────────
// PatientInfoForm
// ────────────────────────────────────────────────────────────────────────────────
export default function PatientInfoForm({ onComplete, onBack }) {
  const [formData, setFormData] = useState({
    birthDate: "",
    sexe: "",
    bloodGroup: "",
    phone: "",
    wilaya: "",
  });
  const [errors, setErrors] = useState({});

  const handleChange = (e) => {
    const { name, value } = e.target;
    setFormData((prev) => ({ ...prev, [name]: value }));
    if (errors[name]) setErrors((prev) => ({ ...prev, [name]: "" }));
  };

  const handleField = (name, value) => {
    setFormData((prev) => ({ ...prev, [name]: value }));
    if (errors[name]) setErrors((prev) => ({ ...prev, [name]: "" }));
  };

  const handleSubmit = () => {
    const newErrors = {};
    if (!formData.birthDate) newErrors.birthDate = "Ce champ est obligatoire";
    if (!formData.sexe) newErrors.sexe = "Ce champ est obligatoire";
    if (!formData.bloodGroup) newErrors.bloodGroup = "Ce champ est obligatoire";
    if (!formData.phone.trim()) {
      newErrors.phone = "Ce champ est obligatoire";
    } else if (formData.phone.length !== 10) {
      newErrors.phone = "Le numéro doit contenir exactement 10 chiffres";
    }
    if (!formData.wilaya) newErrors.wilaya = "Ce champ est obligatoire";
    if (Object.keys(newErrors).length > 0) { setErrors(newErrors); return; }
    onComplete(formData);
  };

  return (
    <div className="min-h-screen bg-[#D1DFEC] flex items-center justify-center p-6 font-sans">
      <div className="bg-white rounded-[24px] shadow-xl w-full max-w-[800px] px-[60px] py-[50px] relative">
        <h2 className="text-[28px] font-bold text-[#0D2644] text-center mb-10">Créer votre compte</h2>

        {/* ── Step Indicator (4 steps – patient) ── */}
        <div className="flex flex-col items-center mb-10">
          <div className="flex items-center w-full max-w-[480px] justify-between relative px-2">
            <div className="absolute left-[30px] right-[67%] h-[2px] bg-[#6492C9] z-0 top-1/2 -translate-y-1/2" />
            <div className="absolute left-[33%] right-[33%] h-[2px] bg-[#6492C9] z-0 top-1/2 -translate-y-1/2" />
            <div className="absolute left-[67%] right-[30px] h-[2px] bg-[#D1DFEC] z-0 top-1/2 -translate-y-1/2" />
            {[true, true, "3", "4"].map((s, i) => (
              <div key={i} className={`w-[32px] h-[32px] rounded-full flex items-center justify-center font-bold text-[13px] z-10 relative ${i < 3 ? "bg-[#6492C9] text-white shadow-sm" : "bg-white border-2 border-[#D1DFEC] text-[#D1DFEC]"}`}>
                {i < 2 ? <Check size={16} strokeWidth={3} /> : s}
              </div>
            ))}
          </div>
        </div>

        {/* ── Section title ── */}
        <div className="flex items-center justify-center mb-10">
          <div className="flex-1 h-px bg-[#D1DFEC]" />
          <span className="mx-4 text-[#365885] text-[15px] font-semibold tracking-wide">Informations personnelles</span>
          <div className="flex-1 h-px bg-[#D1DFEC]" />
        </div>

        {/* ── Form fields ── */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-8 mb-10">

          {/* Date de naissance */}
          <div className="relative group">
            <span className="absolute -top-3 left-4 px-1.5 bg-white text-[13px] font-medium text-[#365885] z-10">Date de naissance</span>
            <input
              type="date" name="birthDate" value={formData.birthDate}
              onChange={handleChange}
              max={new Date().toISOString().split("T")[0]}
              className={`w-full pl-4 pr-4 py-3 rounded-xl border ${errors.birthDate ? "border-red-400" : "border-[#D1DFEC] hover:border-[#89AEDB] focus:border-[#6492C9]"} focus:ring-0 outline-none text-gray-700 text-sm transition-colors`}
              style={{ colorScheme: "light" }}
            />
            {errors.birthDate && <p className="text-red-500 text-[12px] mt-1 ml-1">{errors.birthDate}</p>}
          </div>

          {/* Sexe — RÈGLE 1 */}
          <CustomSelect
            label="Sexe"
            value={formData.sexe}
            options={SEXE_OPTIONS}
            onSelect={(v) => handleField("sexe", v)}
            placeholder="Sélectionner..."
            error={errors.sexe}
          />

          {/* Groupe Sanguin — RÈGLE 1 */}
          <CustomSelect
            label="Groupe Sanguin"
            value={formData.bloodGroup}
            options={BLOOD_GROUPS}
            onSelect={(v) => handleField("bloodGroup", v)}
            placeholder="Ex: A+"
            error={errors.bloodGroup}
          />

          {/* Téléphone — RÈGLE 2 : chiffres uniquement, max 10 */}
          <div className="relative group">
            <span className="absolute -top-3 left-4 px-1.5 bg-white text-[13px] font-medium text-[#365885] z-10">Numéro de téléphone</span>
            <div className="relative">
              <input
                type="text"
                inputMode="numeric"
                name="phone"
                placeholder="0555 123 456"
                value={formData.phone}
                onKeyDown={(e) => {
                  if (!ALLOWED_KEYS.includes(e.key) && !/^\d$/.test(e.key)) e.preventDefault();
                }}
                onChange={(e) => {
                  const onlyNumbers = e.target.value.replace(/\D/g, "");
                  if (onlyNumbers.length <= 10) {
                    setFormData({ ...formData, phone: onlyNumbers });
                    if (errors.phone) setErrors((prev) => ({ ...prev, phone: "" }));
                  }
                }}
                onPaste={(e) => {
                  e.preventDefault();
                  const pasted = e.clipboardData.getData("text").replace(/\D/g, "").slice(0, 10);
                  setFormData({ ...formData, phone: pasted });
                }}
                className={`w-full pl-4 pr-10 py-3 rounded-xl border ${errors.phone ? "border-red-400" : "border-[#D1DFEC] hover:border-[#89AEDB] focus:border-[#6492C9]"} focus:ring-0 outline-none text-gray-700 text-sm transition-colors placeholder:text-[#A0B5CD]`}
              />
              <div className="absolute right-4 top-1/2 -translate-y-1/2 text-[#89AEDB] pointer-events-none">
                <Phone size={18} strokeWidth={1.5} />
              </div>
            </div>
            {!errors.phone && formData.phone.length > 0 && formData.phone.length < 10 && (
              <p className="text-amber-500 text-[11px] mt-1 ml-1">{formData.phone.length}/10 chiffres</p>
            )}
            {formData.phone.length === 10 && (
              <p className="text-green-600 text-[11px] mt-1 ml-1">✓ Numéro complet</p>
            )}
            {errors.phone && <p className="text-red-500 text-[12px] mt-1 ml-1">{errors.phone}</p>}
          </div>

          {/* Wilaya — RÈGLE 1 */}
          <div className="md:col-span-2">
            <CustomSelect
              label="Wilaya"
              value={formData.wilaya}
              options={WILAYAS}
              onSelect={(v) => handleField("wilaya", v)}
              placeholder="Sélectionner votre wilaya"
              error={errors.wilaya}
            />
          </div>
        </div>

        {/* ── Buttons ── */}
        <div className="flex items-center gap-4">
          <button onClick={onBack} className="w-[120px] bg-white border border-[#D1DFEC] hover:border-[#A0B5CD] hover:bg-gray-50 text-[#365885] py-3 rounded-xl text-[15px] font-medium transition-all cursor-pointer flex items-center justify-center gap-1">
            ← Retour
          </button>
          <button onClick={handleSubmit} className="flex-1 bg-[#6492C9] hover:bg-[#304B71] text-white py-3 rounded-xl text-[15px] font-medium transition-all shadow-sm cursor-pointer flex items-center justify-center gap-1">
            Finaliser l'inscription →
          </button>
        </div>
      </div>
    </div>
  );
}
