...
export default function PatientSidebar() {
  ...
  const navItems = [
    ...
    { name: "Medical Profile", icon: HeartPulse },
    { name: "AI Diagnosis", icon: Brain },
    { name: "Appointments", icon: Calendar },
    { name: "Prescriptions", icon: ClipboardList },
    { name: "Pharmacy", icon: Pill },
    { name: "Care Taker", icon: User },
  ];
  ...
  return (
    <aside className="w-auto h-auto bg-white border-r border-[#F1F5F9] flex flex-col py-5 px-6 fixed left-0 top-0 z-50">
      ...
      <div className="flex items-center gap-3 mb-12 px-2 cursor-pointer hover:scale-105 transition-all duration-300 group">
        ...
        <span className="text-[26px] font-bold text-[#8EADE1] tracking-tight">
          MedSmart
        </span>
      </div>
      ...
      <nav className="flex-1 flex flex-col gap-2">
        {navItems.map((item) => (
          ...
          >
            <div
              className={`${active === item.name ? "text-white" : "text-[#64748B] group-hover:text-[#475569]"}`}
            >
              <item.icon
                size={24}
        ...
        <div ref={profileRef} className="relative mt-15">
          ...
          <button
            ...
            className="w-full flex items-center gap-3 p-3 rounded-[16px]
                       bg-[#1E293B] hover:bg-[#2D3748] transition-all cursor-pointer hover:scale-105 "
          >
