...
export default function DoctorSB() {
  ...
  const navItems = [
    ...
    { name: "Patient Profiles", icon: Users },
  ...
  return (
    <aside className="w-auto h-auto bg-white border-r border-[#F1F5F9] flex flex-col py-5 px-6 fixed left-0 top-0 z-50">
      ...
      <div className="flex items-center gap-3 mb-12 px-2 cursor-pointer hover:scale-105 transition-all duration-300 group">
        ...
        <span className="text-[26px] font-bold text-emerald-600 tracking-tight">
          MedSmart
        </span>
      </div>
      ...
      <nav className="flex-1 flex flex-col gap-2">
        {navItems.map((item) => (
          ...
          >
            <div
              className={`${active === item.name ? "text-white" : "text-[#64748B] group-hover:text-emerald-600"}`}
            >
              <item.icon
                size={24}
      ...
      <div className="mt-25 flex flex-col gap-4">
        {/* Profile Block */}
        <div ref={profileRef} className="relative mt-15">
          ...
          <button
            ...
            className="w-full flex items-center gap-3 p-3 rounded-[16px]
                       bg-emerald-900 hover:bg-emerald-950 transition-all cursor-pointer hover:scale-105 "
          >
            {/* Avatar circle with initials */}
            <div
              className="w-9 h-9 rounded-full bg-emerald-500 flex items-center
            ...
            <div className="text-left overflow-hidden">
              ...
              <p className="text-xs text-white/60">Doctor</p>
            </div>
            <ChevronUp
              size={16}
