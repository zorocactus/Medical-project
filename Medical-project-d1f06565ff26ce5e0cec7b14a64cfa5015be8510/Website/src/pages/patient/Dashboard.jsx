    ...
    <>
      ...
      {tab === "book" && (
        <>
          ...
          />
          <h2 className="font-extrabold text-xl mb-6 flex items-center gap-3" style={{ color: c.txt }}>
            <span className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: c.blue + '15' }}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start mb-10">
            ...
            <div className="flex flex-col gap-6">
              {/* 1. Global Search Bar */}
              <div
                className="relative z-20 flex items-center px-5 py-4 rounded-[32px] border transition-all shadow-sm w-full"
                style={{
                  borderColor: searchFocused ? c.blue : c.border,
                  background: c.card,
                  boxShadow: searchFocused ? `0 12px 24px -8px ${c.blue}22` : "none"
                }}
              >
                <Search size={20} className="mr-4" style={{ color: searchFocused ? c.blue : c.txt3 }} />
                <input
                  type="text"
              ...
              <Card dk={dk} className="p-8 rounded-[32px] space-y-6 shadow-sm border h-full" style={{ background: c.card }}>
                <div className="flex items-center justify-between mb-2">
                  <h4 className="text-xs font-black uppercase tracking-[0.2em] opacity-50" style={{ color: c.txt }}>Filtres Avancés</h4>
                  <button onClick={() => { setSearchTerm(""); setSelectedCity(""); setSpecFilter("All"); setSelectedGender("Any Gender"); setStarFilter(1); setSelectedDate(""); }}
                    className="text-[10px] font-bold uppercase tracking-widest hover:opacity-100 opacity-60 transition-opacity" style={{ color: c.blue }}>Réinitialiser</button>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  {/* City select */}
                  <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase tracking-widest opacity-40 ml-1">Wilaya</label>
                    <div className="relative">
                      <button
                        ...
                        style={{ background: c.blue + '05', borderColor: c.border, color: c.txt }}
                      >
                        <span className="truncate">{selectedCity || "Toutes les wilayas"}</span>
                        <ChevronDown size={14} className={`transition-transform duration-300 ${locationOpen ? 'rotate-180' : ''}`} />
                      </button>
                      {locationOpen && (
                        <div className="absolute top-full left-0 right-0 mt-2 z-50 rounded-2xl shadow-2xl border overflow-hidden animate-in fade-in zoom-in-95"
                          style={{ background: c.card, borderColor: c.border, maxHeight: '200px', overflowY: 'auto' }}>
                          {CITIES.map(city => (
                            <button key={city} onClick={() => { setSelectedCity(city); setLocationOpen(false); }}
                              className="w-full px-4 py-3 text-left text-xs font-bold hover:bg-opacity-50 transition-all border-b last:border-0"
                              style={{ background: selectedCity === city ? c.blue + '15' : 'transparent', color: selectedCity === city ? c.blue : c.txt, borderColor: c.border }}>
                              {city}
                            </button>
                          ))}
                        </div>
                  ...
                  {/* Specialty select */}
                  <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase tracking-widest opacity-40 ml-1">Spécialité</label>
                    <div className="relative">
                      <button
                        ...
                        style={{ background: c.blue + '05', borderColor: c.border, color: c.txt }}
                      >
                        <span className="truncate">{specFilter === "All" ? "Toute Spécialité" : specFilter}</span>
                        <ChevronDown size={14} className={`transition-transform duration-300 ${specOpen ? 'rotate-180' : ''}`} />
                      </button>
                      {specOpen && (
                        <div className="absolute top-full left-0 right-0 mt-2 z-50 rounded-2xl shadow-2xl border overflow-hidden animate-in fade-in zoom-in-95"
                          style={{ background: c.card, borderColor: c.border, maxHeight: '200px', overflowY: 'auto' }}>
                          {SPECIALTIES.map(s => (
                            <button key={s} onClick={() => { setSpecFilter(s); setSpecOpen(false); }}
                              className="w-full px-4 py-3 text-left text-xs font-bold hover:bg-opacity-50 transition-all border-b last:border-0"
                              style={{ background: specFilter === s ? c.blue + '15' : 'transparent', color: specFilter === s ? c.blue : c.txt, borderColor: c.border }}>
                  ...
                  {/* Date Filter */}
                  <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase tracking-widest opacity-40 ml-1">Calendrier (Date)</label>
                    <div
                      onClick={() => dateInputRef.current?.showPicker?.()}
                      className="relative flex items-center gap-2 px-4 py-3.5 rounded-2xl border text-sm font-bold cursor-pointer transition-all"
                      style={{ background: c.blue + '05', borderColor: c.border, color: selectedDate ? c.txt : c.txt3 }}
                    >
                  ...
                  {/* Rating Filter */}
                  <div className="space-y-2">
                    <label className="text-[10px] font-black uppercase tracking-widest opacity-40 ml-1">Note Minimale</label>
                    <div className="flex items-center justify-between gap-1 p-1 rounded-2xl border" style={{ background: c.blue + '05', borderColor: c.border }}>
                      {[1, 2, 3, 4, 5].map(star => (
                        <button key={star} onClick={() => setStarFilter(star)}
                          ...
                          style={{
                            background: star <= starFilter ? '#E8A838' + '22' : 'transparent',
                            color: star <= starFilter ? '#E8A838' : c.border,
                          }}>
                          ★
                        </button>
                      ))}
                ...
                <div className="space-y-2 pt-2">
                  <label className="text-[10px] font-black uppercase tracking-widest opacity-40 ml-1">Genre du médecin</label>
                  <div className="flex p-1.5 rounded-2xl border transition-all" style={{ background: c.blue + '05', borderColor: c.border }}>
                    {["Any Gender", "Masculin", "Féminin"].map(opt => (
                      <button key={opt} onClick={() => setSelectedGender(opt)}
                        className="flex-1 py-2.5 rounded-xl text-xs font-black transition-all"
                        style={{ background: selectedGender === opt ? c.blue : 'transparent', color: selectedGender === opt ? '#fff' : c.txt2 }}>
                        {opt === "Any Gender" ? "Tous" : opt}
                      </button>
                    ))}
                  </div>
                </div>
            ...
            <div className="h-full min-h-[500px]">
              {/* GPS Map Card */}
              <Card dk={dk} className="p-0 rounded-[32px] overflow-hidden shadow-sm border relative h-full w-full" style={{ borderColor: c.border }}>
                <div className="absolute top-4 left-4 z-10 px-3 py-1.5 rounded-xl bg-white/90 dark:bg-slate-900/90 backdrop-blur-md shadow-md flex items-center gap-2 border" style={{ borderColor: c.border }}>
                  <div className="w-2.5 h-2.5 rounded-full bg-red-500 animate-pulse" />
                  <span className="text-[10px] font-bold text-slate-800 dark:text-white uppercase tracking-wider">Localisation GPS</span>
                </div>
                <iframe
                  src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d12791.73461476628!2d3.04561!3d36.75618!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x128fafda11111111%3A0x1111111111111111!2sAlger!5e0!3m2!1sfr!2sdz!4v1650000000000!5m2!1sfr!2sdz"
                  width="100%"
                  height="100%"
                  style={{ border: 0 }}
                  allowFullScreen=""
                  ...
                  className="grayscale-0 contrast-[1.1]"
                />
              </Card>
            </div>
          </div>

          {/* 3. Appointment Booking Panel (Appears when a doctor is selected) */}
          {selectedDoctor && (
            <div
              className="rounded-[40px] border overflow-hidden shadow-2xl transition-all duration-500 animate-in fade-in slide-in-from-top-4"
                style={{ background: c.card, borderColor: c.blue + '44' }}
              >
                {/* Doctor header (Premium) */}
                <div
                  className="p-8 border-b flex items-center justify-between gap-4 bg-opacity-50 backdrop-blur-md"
                  style={{
                    borderColor: c.border,
                ...
                >
                  <div className="flex items-center gap-6">
                    <div
                      className="w-20 h-20 rounded-3xl flex items-center justify-center text-white font-black text-2xl shadow-2xl shrink-0"
                      style={{
                    ...
                    <div>
                      <h3 className="font-black text-2xl mb-1" style={{ color: c.txt }}>
                        {selectedDoctor.name}
                      </h3>
                      <div className="flex items-center gap-3 flex-wrap">
                        <span className="px-3 py-1 rounded-full text-[10px] font-black bg-blue-500/10 text-blue-500 uppercase tracking-wider">{selectedDoctor.spec}</span>
                        <div className="flex items-center gap-1.5 text-sm opacity-60 font-medium" style={{ color: c.txt }}>
                           <MapPin size={14} /> {selectedDoctor.loc}
                        </div>
                      </div>
                    </div>
                  </div>
                  <button
                    onClick={() => { setSelectedDoctor(null); setCalDay(null); setCalSlot(null); }}
                    className="w-12 h-12 rounded-full flex items-center justify-center transition-all hover:rotate-90 hover:bg-slate-100 dark:hover:bg-slate-800 shadow-sm"
                  >
                    <X size={24} style={{ color: c.txt3 }} />
                  </button>
                </div>

                <div className="grid grid-cols-1 xl:grid-cols-12 content-stretch">
                  {/* Left Column: Calendar (7 cols) */}
                  <div className="xl:col-span-7 p-8 border-r" style={{ borderColor: c.border }}>
                    <div className="flex items-center justify-between mb-8">
                      <h4 className="font-black text-lg flex items-center gap-2" style={{ color: c.txt }}>
                         <Calendar size={20} className="text-blue-500" /> Choisir une Date
                      </h4>
                      <div className="flex items-center gap-2">
                        <button onClick={() => setCalMonth(new Date(year, month - 1, 1))}
                          className="w-10 h-10 rounded-xl flex items-center justify-center border transition-all hover:bg-opacity-80"
                          style={{ borderColor: c.border, background: c.card }}><ChevronRight size={18} style={{ color: c.txt2, transform: "rotate(180deg)" }} /></button>
                        <div className="px-4 py-2 rounded-xl border font-black text-sm min-w-[140px] text-center" style={{ borderColor: c.border, background: c.card, color: c.txt }}>
                          {monthNames[month]} {year}
                        </div>
                        <button onClick={() => setCalMonth(new Date(year, month + 1, 1))}
                          className="w-10 h-10 rounded-xl flex items-center justify-center border transition-all hover:bg-opacity-80"
                          style={{ borderColor: c.border, background: c.card }}><ChevronRight size={18} style={{ color: c.txt2 }} /></button>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-7 gap-3">
                      ...
                      {days.map((d) => {
                        ...
                        return (
                          <button key={d} onClick={() => { setCalDay(d); setCalSlot(null); }}
                            className="aspect-square rounded-2xl flex flex-col items-center justify-center relative transition-all group font-black text-sm border-2"
                            style={{
                              background: isSel ? c.blue : isToday ? c.blue + '08' : "transparent",
                              color: isSel ? "#fff" : isToday ? c.blue : c.txt,
                              borderColor: isSel ? c.blue : isToday ? c.blue + '33' : 'transparent',
                            }}>
                            {d}
                            {isToday && !isSel && <div className="w-1.5 h-1.5 rounded-full bg-blue-500 absolute bottom-2" />}
                          </button>
                        );
                      })}
                    </div>
                  ...
                  <div className="xl:col-span-5 p-8" style={{ background: dk ? "#ffffff03" : "#00000002" }}>
                    {!calDay ? (
                      <div className="h-full flex flex-col items-center justify-center opacity-40 text-center">
                        ...
                        <p className="text-base font-bold">Sélectionnez d'abord une date pour voir les créneaux</p>
                      </div>
                    ) : (
                      <div className="h-full flex flex-col">
                         <h4 className="font-black text-lg mb-6 flex items-center gap-2" style={{ color: c.txt }}>
                           <Clock size={20} className="text-blue-500" /> Créneaux Disponibles
                        </h4>
                        <div className="flex-1 overflow-y-auto pr-2 space-y-6 max-h-[360px] custom-scrollbar">
                           {slotsLoading ? (
                              <div className="flex items-center justify-center py-12 opacity-50"><div className="w-8 h-8 border-4 border-t-transparent rounded-full animate-spin" style={{ borderColor: c.blue, borderTopColor: "transparent" }} /></div>
                           ) : availableSlots.length === 0 ? (
                              <div className="py-12 text-center opacity-40"><p className="font-bold">Aucun créneau ce jour</p></div>
                           ) : (
                             <>
                               {morningSlots.length > 0 && (
                                 <div className="space-y-3">
                                   <p className="text-[10px] font-black uppercase tracking-widest opacity-40 flex items-center gap-2"><Sun size={14} /> Matin</p>
                                   <div className="grid grid-cols-3 gap-2">
                                     {morningSlots.map(s => (
                                       <button key={s.time} onClick={() => setCalSlot(s.time)}
                                         className="py-3.5 rounded-2xl text-[13px] font-black border-2 transition-all"
                                         style={{ background: calSlot === s.time ? c.blue : c.card, color: calSlot === s.time ? "#fff" : c.txt, borderColor: calSlot === s.time ? c.blue : c.border }}>{s.time}</button>
                                     ))}
                                   </div>
                                 </div>
                               )}
                               {afternoonSlots.length > 0 && (
                                 <div className="space-y-3">
                                   <p className="text-[10px] font-black uppercase tracking-widest opacity-40 flex items-center gap-2"><Moon size={14} /> Après-midi</p>
                                   <div className="grid grid-cols-3 gap-2">
                        ...
                        <div className="mt-8 pt-8 border-t" style={{ borderColor: c.border }}>
                          <button disabled={!calSlot} onClick={() => handleBook(selectedDoctor)}
                            className="w-full py-5 rounded-[24px] text-sm font-black text-white transition-all shadow-2xl disabled:opacity-30 flex items-center justify-center gap-3 active:scale-95 group"
                            style={{ background: `linear-gradient(135deg, ${c.blue}, #304B71)`, boxShadow: `0 20px 40px -12px ${c.blue}66` }}>
                            CONFIRMER LA RÉSERVATION <ArrowRight size={18} className="group-hover:translate-x-1 transition-transform" />
                          </button>
                        </div>
                      </div>
                    )}
            ...
            {/* 4. Doctor Results Grid (Bottom Section) */}
            <div className="space-y-6 pt-4">
              <div className="flex items-center justify-between">
                 <h3 className="font-black text-xl flex items-center gap-3" style={{ color: c.txt }}>
                   Médecins disponibles <span className="text-sm opacity-40 font-bold">({filteredDoctors.length})</span>
                 </h3>
              </div>
              
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6" ref={docListRef}>
                {filteredDoctors.map((doc) => {
                  const live = getLiveRating(doc);
                  return (
                    <Card key={doc.id} dk={dk}
                      className="group flex flex-col p-6 rounded-[36px] hover:shadow-2xl transition-all duration-500 border-0 shadow-md relative overflow-hidden"
                      style={{ background: c.card }}
                    >
                      {/* Avatar & Specs */}
                      <div className="flex items-start justify-between mb-6 relative z-10">
                        <div className="w-16 h-16 rounded-[24px] flex items-center justify-center text-white font-black text-xl shadow-xl group-hover:scale-110 transition-transform duration-500"
                          style={{ background: `linear-gradient(135deg, ${doc.color}, ${doc.color}cc)` }}>{doc.initials}</div>
                        <div className="flex flex-col items-end gap-1.5">
                           <div className="flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] font-black" style={{ background: "#E8A83815", color: "#E8A838" }}>
                            ★ {live.rating}
                          </div>
                          <span className="text-[10px] font-bold opacity-30 uppercase tracking-widest">{live.reviews} avis</span>
                        </div>
                      </div>

                      <div className="flex-1 relative z-10">
                        <h3 className="font-bold text-lg mb-0.5 truncate" style={{ color: c.txt }}>{doc.name}</h3>
                        <p className="text-[11px] font-black uppercase tracking-widest mb-5" style={{ color: c.blue }}>{doc.spec}</p>

                        <div className="space-y-3 mb-8">
                          <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-xl flex items-center justify-center shrink-0" style={{ background: c.blue + '08' }}><MapPin size={14} style={{ color: c.blue }} /></div>
                            <p className="text-[11px] font-bold opacity-70 leading-tight" style={{ color: c.txt }}>{doc.loc}</p>
                          </div>
                          <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-xl flex items-center justify-center shrink-0" style={{ background: c.blue + '08' }}><Activity size={14} style={{ color: c.blue }} /></div>
                            <p className="text-[11px] font-bold opacity-70" style={{ color: c.txt }}>{doc.exp} ans d'expérience</p>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="relative z-10 pt-2">
                        <button onClick={() => openCalendar(doc)}
                          className="w-full py-4 rounded-2xl text-[11px] font-black text-white transition-all shadow-lg active:scale-95 flex items-center justify-center gap-2 group/btn"
                          style={{ background: `linear-gradient(135deg, ${c.blue}, #304B71)`, boxShadow: `0 12px 24px -8px ${c.blue}66` }}>
                          PRENDRE RDV <ArrowRight size={14} className="group-hover/btn:translate-x-1 transition-transform" />
                        </button>
                      </div>

                      {/* Accent decoration */}
                      <div className="absolute -bottom-6 -right-6 w-24 h-24 rounded-full opacity-0 group-hover:opacity-10 transition-opacity duration-700" style={{ background: doc.color }} />
                    </Card>
                  );
                })}
              </div>
              ...
              {filteredDoctors.length === 0 && (
                 <div className="py-20 text-center">
                   <EmptyState dk={dk} icon={Search} title="Désolé, aucun médecin trouvé" message="Modifiez vos filtres ou la zone de recherche pour voir de nouveaux résultats." />
                 </div>
              )}
            </div>
...
function PrescriptionsPage({ dk }) {
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 content-stretch">
        {filteredRxList.length === 0 ? (
          ...
          filteredRxList.map((rx) => {
            ...
            return (
              <Card key={rx.id} dk={dk} className="interactive-card hover:shadow-md transition-shadow group">
      ...
      {downloading && (
        ...
        >
          <span className="w-5 h-5 border-2 border-white border-t-transparent shrink-0 rounded-full animate-spin"></span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
            {PHARMACY_ITEMS.map((item) => (
              <Card
                key={item.id}
                dk={dk}
                className="interactive-card hover:shadow-md transition-shadow group"
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            {ctLoading ? (
              ...
              return (
                <Card key={ct.id} dk={dk} className="interactive-card hover:shadow-md transition-shadow group">
                  <div className="flex gap-4 flex-wrap">
                    <div className="w-14 h-14 rounded-2xl flex items-center justify-center text-white font-bold text-lg shrink-0"
                    ...
                    <div className="flex flex-col gap-2 shrink-0 justify-center">
                      <button onClick={() => setProfileModal(ct)}
                        className="btn-secondary text-xs font-semibold px-4 py-2 rounded-xl border transition-colors"
                        style={{ color: c.txt2, borderColor: c.border }}>
                        Voir Profil
                      </button>
                      <button onClick={() => handleAssign(ct)}
                        className="btn-primary text-xs font-bold px-4 py-2 rounded-xl text-white shadow-md"
