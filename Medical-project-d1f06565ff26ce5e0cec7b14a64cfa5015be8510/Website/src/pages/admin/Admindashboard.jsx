...
// ─── Shared Components ────────────────────────────────────────────────────────
function Card({ children, className = "", style = {}, dk, interactive = false }) {
  const c = dk ? T.dark : T.light;
  return (
    <div className={`rounded-2xl border ${interactive ? "interactive-card" : ""} ${className}`}
...
function useRipple() {
  const createRipple = (e) => {
    ...
    `;
    const prev = btn.querySelector(".ripple-el");
    if (prev) prev.remove();
...
function KpiCard({ label, value, sub, icon: Icon, color, trend, trendLabel, dk }) {
  ...
  const up = trend >= 0;
  return (
    <Card dk={dk} interactive style={{ padding: 20, cursor: "pointer" }}>
      <div className="flex items-start justify-between mb-3">
        <div className="kpi-icon w-11 h-11 rounded-xl flex items-center justify-center"
    ...
    <>
      ...
      <div className="mb-6 flex items-start justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-3">
          <button className="btn-secondary flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold border"
            style={{ borderColor: c.border, color: c.txt2 }}>
            <Download size={14} /> Export rapport
          </button>
          <button className="btn-primary flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold text-white"
        ...
        <Card dk={dk} style={{ padding: 20, gridColumn: "span 2" }}>
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-2">
              {["1S", "1M", "3M", "1A"].map((p, i) => (
                <button key={p} className="btn-filter px-2.5 py-1 rounded-lg text-xs font-semibold"
            ...
            {[38, 52, 45, 61, 58, 74, 69, 82, 77, 91, 86, 96].map((h, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-1">
                <div
                  ...
                  style={{
                    height: `${h}%`,
                    background: i === 11 ? c.blue : (dk ? "rgba(74,111,165,0.3)" : "#D5E3F5"),
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-5">
        ...
        <Card dk={dk} style={{ padding: 20 }}>
          ...
          <div className="space-y-2">
            {WILAYAS_DATA.map(w => (
              <div key={w.name} className="progress-row flex items-center gap-3">
                ...
                <div className="flex-1 h-2 rounded-full overflow-hidden" style={{ background: c.blueLight }}>
                  <div className="h-full rounded-full transition-all duration-500" style={{ width: `${(w.users / 1248) * 100}%`, background: c.blue }} />
                </div>
    ...
    <>
          ...
          {selected.length > 0 && (
            <div className="flex items-center gap-2 px-3 py-2 rounded-xl border" style={{ borderColor: c.border }}>
              ...
              <button onClick={() => selected.forEach(id => suspend(id))}
                className="btn-filter text-xs font-bold px-2 py-1 rounded-lg" style={{ background: c.amberLight, color: c.amber }}>
                Suspendre
              </button>
              <button onClick={() => { selected.forEach(id => deleteUser(id)); setSelected([]); }}
                className="btn-filter text-xs font-bold px-2 py-1 rounded-lg" style={{ background: c.redLight, color: c.red }}>
                Supprimer
              </button>
            </div>
          )}
      ...
      <Card dk={dk} className="mb-5" style={{ padding: "12px 16px" }}>
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <div className="flex items-center gap-1 flex-wrap">
            {[["all", "Tous"], ["patient", "Patients"], ["médecin", "Médecins"], ["pharmacien", "Pharmaciens"], ["garde-malade", "Gardes"]].map(([val, label]) => (
              <button key={val} onClick={() => handleFilterChange(setRoleFilter)(val)}
                className="btn-filter px-3 py-1.5 rounded-xl text-xs font-semibold border"
          ...
          <button className="btn-secondary flex items-center gap-2 px-3 py-2 rounded-xl text-sm border"
            style={{ borderColor: c.border, color: c.txt2 }}>
                      ...
                      <div className="flex items-center gap-1.5">
                        {u.status === "pending" && (
                          <button onClick={() => approve(u.id)}
                            className="btn-icon w-7 h-7 rounded-lg flex items-center justify-center"
                            title="Approuver" style={{ background: c.greenLight }}>
                            <Check size={13} style={{ color: c.green }} />
                          </button>
                        )}
                        <button onClick={() => suspend(u.id)}
                          className="btn-icon w-7 h-7 rounded-lg flex items-center justify-center border"
                    ...
                    <div className="flex flex-wrap gap-2">
                      {pro.docs.map(doc => (
                        <button key={doc.key}
                          ...
                          className="doc-btn flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold border"
      ...
      <div className="flex gap-2 mb-5 flex-wrap">
        {[
          ...
          <button key={val} onClick={() => setTypeFilter(val)}
            className="btn-filter flex items-center gap-1.5 px-4 py-2 rounded-full text-xs font-bold border"
      ...
      <Card dk={dk} style={{ padding: 0, overflow: "hidden" }}>
        <div className="divide-y" style={{ borderColor: c.border }}>
          {filtered.map(log => {
            const lm = LOG_COLORS[log.type];
            return (
              <div key={log.id} className="audit-row flex items-start gap-4 px-5 py-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <Card dk={dk} style={{ padding: 20 }}>
          ...
          <div className="space-y-4">
            ...
            <button className="btn-primary w-full py-2.5 rounded-xl text-sm font-semibold text-white" style={{ background: c.blue }}>
              Enregistrer
            </button>
          </div>
        </Card>
        ...
        * { transition: background-color 0.2s, border-color 0.2s; }
        ...
        @media (hover: hover) {
          ...
          .interactive-card:hover {
            ...
            box-shadow: 0 20px 40px -12px rgba(74,111,165,0.28) !important;
            border-color: ${c.blue} !important;
          }
          .interactive-card:hover .kpi-icon {
        ...
        .interactive-card:active {
          ...
          background: ${dk ? "rgba(74,111,165,0.08)" : "#f0f6ff"} !important;
        }
        .kpi-icon { transition: transform 0.2s ease; }

        /* ── Navigation links ── */
        .nav-link {
          transition: background 0.18s ease, color 0.18s ease, transform 0.15s ease, box-shadow 0.18s ease !important;
        }
        @media (hover: hover) {
          .nav-link:not(.active-nav):hover {
            background: rgba(90,135,197,0.14) !important;
            color: #5A87C5 !important;
            transform: translateY(-1px);
            box-shadow: 0 4px 12px rgba(90,135,197,0.15);
          }
        }
        .nav-link:active {
        ...
        @media (hover: hover) {
          .btn-primary:hover {
            ...
            box-shadow: 0 8px 24px rgba(74,111,165,0.38) !important;
            opacity: 0.94;
          }
        }
        .btn-primary:active {
        ...
        @media (hover: hover) {
          .btn-secondary:hover {
            ...
            box-shadow: 0 4px 12px rgba(74,111,165,0.14) !important;
            background: ${dk ? "rgba(255,255,255,0.05)" : "rgba(74,111,165,0.06)"} !important;
          }
        }
        ...
        /* ── Action icon buttons (approve/delete etc.) ── */
        .btn-icon {
        ...
        /* ── Filter / tab buttons ── */
        .btn-filter {
          transition: transform 0.15s ease, background 0.15s, color 0.15s, border-color 0.15s, box-shadow 0.15s !important;
        }
        @media (hover: hover) {
          .btn-filter:hover {
            transform: translateY(-1px) !important;
            box-shadow: 0 3px 10px rgba(74,111,165,0.15) !important;
          }
        }
        .btn-filter:active { transform: scale(0.95) !important; }
        ...
        .chart-bar:active { filter: brightness(0.9); }

        /* ── Progress bar rows (wilaya, role) ── */
        .progress-row {
        ...
        @media (hover: hover) {
          .progress-row:hover { opacity: 0.85; transform: translateX(3px); }
        }
        ...
        @media (hover: hover) {
          .table-row:hover { background: ${dk ? "rgba(255,255,255,0.025)" : "#F5F9FF"} !important; }
        }
        ...
        .pd-item { color:${c.txt2}; background:transparent; transition:background 0.15s,color 0.15s,transform 0.15s; }
        .pd-item:hover { background:${dk ? "rgba(255,255,255,0.05)" : "rgba(90,135,197,0.08)"}; color:${dk ? "#E8F0FA" : c.blue}; transform: translateX(2px); }
        .pd-item-danger { color:#EF4444; background:transparent; transition:background 0.15s,transform 0.15s; }
        .pd-item-danger:hover { background:rgba(239,68,68,0.1); transform: translateX(2px); }

        /* ── Document/tag buttons in validation ── */
        .doc-btn {
          transition: transform 0.15s ease, box-shadow 0.15s ease, background 0.15s !important;
        }
        @media (hover: hover) {
          .doc-btn:hover { transform: translateY(-2px) scale(1.04); box-shadow: 0 4px 12px rgba(74,111,165,0.18); }
        }
        ...
        @media (hover: hover) {
          .audit-row:hover { padding-left: 28px !important; background: ${dk ? "rgba(255,255,255,0.025)" : "#FAFBFD"} !important; }
        }
        ...
        /* ── Mobile tap feedback ── */
        @media (hover: none) {
          .interactive-card:active { transform: scale(0.97) !important; background: ${dk ? "rgba(74,111,165,0.08)" : "#f0f6ff"} !important; }
          .btn-primary:active, .btn-secondary:active, .btn-filter:active { transform: scale(0.95) !important; }
          .nav-link:active { background: rgba(90,135,197,0.16) !important; }
        }
      `}</style>
          ...
          <div className="hidden lg:flex items-center justify-center gap-1 flex-1 overflow-x-auto" style={{ scrollbarWidth: "none" }}>
            {NAV.map(item => (
              <button key={item.id} onClick={() => setPage(item.id)}
                className={`nav-link${page === item.id ? " active-nav" : ""} relative flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium whitespace-nowrap`}
                style={{ color: page === item.id ? "#fff" : c.txt2, background: page === item.id ? c.blue : "transparent" }}>
                <item.icon size={14} style={{ opacity: 0.8 }} />
                {item.label}
                {item.badge && (
